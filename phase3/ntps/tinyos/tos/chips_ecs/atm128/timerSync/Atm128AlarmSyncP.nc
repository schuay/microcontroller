...
generic module Atm128AlarmSyncP(typedef precision, int divider) @safe() {
  provides {
    interface Init;
...
implementation
{
  ...
  command error_t Init.init() {
	...
	call TimerSync.setTimer0Synchronous();
	x.flat = 0;
	x.bits.cs = divider;
  ...
  */
  void setOcr0(uint8_t n) {
    while (call TimerSync.compareBusy())
      ;
    if (n == call Timer.get())
      n++;
  ...
  void setInterrupt() {
	...
	if ((interrupt_in != 0 && interrupt_in < MINDT) || (tifr & (1 << OCF0))) {
	  if (interrupt_in < MINDT) {
	    ...
	    dbg("Atm128AlarmSyncP", "Atm128AlarmSyncP: OCF set.\n");
	  }
