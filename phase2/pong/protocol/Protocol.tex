...
\begin{displaymath}
    t_{CB} = \frac{T_{time}}{DG_{length} - \mathit{Buffersize} + 1} = \frac{10 * DG_{length}}{\mathit{BAUD} * (DG_{length} - \mathit{Buffersize} + 1)}
\end{displaymath}


For $\mathit{BAUD}=10^{6}bit/sec$, $\mathit{Buffersize}=49Byte$, and $DG_{length}=64Byte$, $t_{CB} = 4 * 10^{-5}$ seconds, or 40 $\mu$s.
